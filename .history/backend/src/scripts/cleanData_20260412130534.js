...
// Allowed fields in the final output
const ALLOWED_FIELDS = ["name", "email", "enrollment_date", "course", "status"];
...
function parseDate(dateStr) {
  ...
  // Fallback: try native Date parsing for text dates like "March 5, 2024"
...
function cleanData() {
  ...
  // Step 1: Keep only allowed fields
  const stripped = raw.map((record) => {
    const cleaned = {};
